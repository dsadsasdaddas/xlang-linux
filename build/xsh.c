...
int32_t main(int argc, char** argv) {
    ...
    __xlang_argv_g = argv;
    while (true) {
        const char * cmd = __xlang_read_line();
        if (((int32_t)strlen(cmd) == 0)) {
            return 0;
        }
        if ((strcmp(cmd, "pwd") == 0)) {
            printf("%s", __xlang_getcwd());
            printf("%s", "\n");
        } else {
            if ((__xlang_str_find(cmd, "cd ") == 0)) {
                chdir((__xlang_str_slice(cmd, 3, (int32_t)strlen(cmd))));
            } else {
                system(cmd);
            }
        }
