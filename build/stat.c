...
int32_t main(int argc, char** argv) {
    ...
    const char * path = __xlang_argv_g[1];
    if (__xlang_file_exists(path)) {
        if (__xlang_is_dir(path)) {
            printf("%s", __xlang_str_concat(path, ": directory\n"));
        } else {
            int32_t size = __xlang_file_size(path);
            printf("%s", __xlang_str_concat(__xlang_str_concat(__xlang_str_concat(path, ": file, "), __xlang_int_to_str(size)), " bytes\n"));
        }
    } else {
        printf("%s", __xlang_str_concat(path, ": not found\n"));
        return 1;
    }
    return 0;
}
