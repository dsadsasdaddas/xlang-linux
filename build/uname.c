...
const char * strip_nl(const char * s);
...
const char * strip_nl(const char * s) {
    ...
    int32_t e = n;
    if ((n > 0)) {
        if (((int32_t)(unsigned char)(s[(n - 1)]) == 10)) {
            (e = (n - 1));
        }
    }
    return __xlang_str_slice(s, 0, e);
}

int32_t main(int argc, char** argv) {
    __xlang_argc_g = argc;
    __xlang_argv_g = argv;
    const char * sysname = strip_nl(__xlang_read_file("/proc/sys/kernel/ostype"));
    const char * release = strip_nl(__xlang_read_file("/proc/sys/kernel/osrelease"));
    printf("%s", __xlang_str_concat(__xlang_str_concat(sysname, " "), release));
    printf("%s", "\n");
    return 0;
}
