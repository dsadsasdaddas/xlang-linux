...
int32_t main(int argc, char** argv) {
    ...
    Vec_String a = split_lines(__xlang_read_file(__xlang_argv_g[1]));
    Vec_String b = split_lines(__xlang_read_file(__xlang_argv_g[2]));
    int32_t an = ((int32_t)(a).len);
    int32_t bn = ((int32_t)(b).len);
    int32_t i = 0;
    int32_t j = 0;
    while (((i < an) || (j < bn))) {
        if ((i >= an)) {
            ...
            if ((j >= bn)) {
                ...
                int32_t cmp = (int32_t)strcmp(a.data[i], b.data[j]);
                if ((cmp < 0)) {
                    printf("%s", a.data[i]);
                    printf("%s", "\n");
                    (i = (i + 1));
