...
int32_t main(int argc, char** argv) {
    ...
    bool invert = false;
    int32_t file_idx = 0;
    if ((strcmp(__xlang_argv_g[1], "-v") == 0)) {
        if (((__xlang_argc_g) < 3)) {
            printf("%s\n", "usage: grep -v <pattern> [file]");
            return 1;
        }
    ...
    while ((i < n)) {
        if (((int32_t)(unsigned char)(s[i]) == 10)) {
            const char * line = __xlang_str_slice(s, start, i);
            bool found = (__xlang_str_find(line, pat) >= 0);
            if ((found != invert)) {
                printf("%s", line);
                printf("%s", "\n");
                (matched = (matched + 1));
    ...
    if ((start < n)) {
        const char * line = __xlang_str_slice(s, start, n);
        bool found = (__xlang_str_find(line, pat) >= 0);
        if ((found != invert)) {
            printf("%s", line);
            printf("%s", "\n");
