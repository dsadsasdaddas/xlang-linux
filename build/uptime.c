...
int32_t main(int argc, char** argv) {
    __xlang_argc_g = argc;
    __xlang_argv_g = argv;
    const char * u = __xlang_read_file("/proc/uptime");
    int32_t n = (int32_t)strlen(u);
    int32_t i = 0;
    int32_t val = 0;
    while ((i < n)) {
        int32_t c = (int32_t)(unsigned char)(u[i]);
        if ((c >= 48)) {
            if ((c <= 57)) {
                (val = ((val * 10) + (c - 48)));
            }
    ...
    printf("%s", __xlang_str_concat(__xlang_str_concat("up ", __xlang_int_to_str(val)), " seconds\n"));
    return 0;
}
