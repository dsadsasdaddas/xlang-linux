...
void __xlang_vec_push_String(Vec_String *v, const char * x) {
    if (v->len == v->cap) {
        v->cap = v->cap ? v->cap * 2 : 4;
        v->data = (const char * *)realloc(v->data, v->cap * sizeof(const char *));
    }
    v->data[v->len++] = x;
}

int32_t quicksort(Vec_String lines, int32_t lo, int32_t hi, bool reverse);
int32_t main(int argc, char** argv);

int32_t quicksort(Vec_String lines, int32_t lo, int32_t hi, bool reverse) {
    if ((lo < hi)) {
        const char * pivot = lines.data[hi];
        int32_t i = (lo - 1);
        int32_t j = lo;
        while ((j < hi)) {
            int32_t cmp = (int32_t)strcmp(lines.data[j], pivot);
        ...
        (i = (i + 1));
        const char * tmp2 = lines.data[i];
        (lines.data[i] = lines.data[hi]);
        (lines.data[hi] = tmp2);
...
int32_t main(int argc, char** argv) {
    ...
    const char * s = "";
    if (((__xlang_argc_g) >= 2)) {
        if ((strcmp(__xlang_argv_g[1], "-r") == 0)) {
            (reverse = true);
            if (((__xlang_argc_g) >= 3)) {
                (s = __xlang_read_file(__xlang_argv_g[2]));
            } else {
                (s = __xlang_read_stdin());
            }
        } else {
            (s = __xlang_read_file(__xlang_argv_g[1]));
        }
    } else {
        (s = __xlang_read_stdin());
    }
    Vec_String lines = (Vec_String){ .data = 0, .len = 0, .cap = 0 };
    int32_t n = (int32_t)strlen(s);
    int32_t start = 0;
    int32_t i = 0;
    while ((i < n)) {
        if (((int32_t)(unsigned char)(s[i]) == 10)) {
            __xlang_vec_push_String(&lines, __xlang_str_slice(s, start, i));
            (start = (i + 1));
        }
        (i = (i + 1));
    }
    if ((start < n)) {
        __xlang_vec_push_String(&lines, __xlang_str_slice(s, start, n));
    }
    int32_t count = ((int32_t)(lines).len);
    if ((count > 0)) {
        quicksort(lines, 0, (count - 1), reverse);
    }
    int32_t k = 0;
    while ((k < count)) {
        printf("%s", lines.data[k]);
        printf("%s", "\n");
        (k = (k + 1));
    }
    return 0;
