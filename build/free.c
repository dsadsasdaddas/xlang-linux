...
int32_t extract_meminfo(const char * m, const char * key);
int32_t main(int argc, char** argv);

int32_t extract_meminfo(const char * m, const char * key) {
    int32_t pos = __xlang_str_find(m, key);
    if ((pos < 0)) {
        return 0;
    }
    int32_t n = (int32_t)strlen(m);
    int32_t i = (pos + (int32_t)strlen(key));
    while ((i < n)) {
        int32_t c = (int32_t)(unsigned char)(m[i]);
        if ((c >= 48)) {
            if ((c <= 57)) {
                break;
            }
...
int32_t main(int argc, char** argv) {
    __xlang_argc_g = argc;
    __xlang_argv_g = argv;
    const char * m = __xlang_read_file("/proc/meminfo");
    int32_t total = extract_meminfo(m, "MemTotal:");
    int32_t avail = extract_meminfo(m, "MemAvailable:");
    printf("%s", __xlang_str_concat("total: ", __xlang_int_to_str(total)));
    printf("%s", " kB  ");
    printf("%s", __xlang_str_concat("available: ", __xlang_int_to_str(avail)));
    printf("%s", " kB\n");
    return 0;
}
