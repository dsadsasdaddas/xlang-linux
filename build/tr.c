...
void __xlang_vec_push_i32(Vec_i32 *v, int32_t x) {
    if (v->len == v->cap) {
        v->cap = v->cap ? v->cap * 2 : 4;
        v->data = (int32_t *)realloc(v->data, v->cap * sizeof(int32_t));
    }
    v->data[v->len++] = x;
}
...
int32_t main(int argc, char** argv) {
    ...
    if ((strcmp(__xlang_argv_g[1], "-d") == 0)) {
        ...
        (set1 = __xlang_argv_g[1]);
        (set2 = __xlang_argv_g[2]);
        if (((__xlang_argc_g) >= 4)) {
            (s = __xlang_read_file(__xlang_argv_g[3]));
        } else {
            (s = __xlang_read_stdin());
    ...
    if (delete_mode) {
        Vec_i32 table = (Vec_i32){ .data = 0, .len = 0, .cap = 0 };
        int32_t k = 0;
        while ((k < 256)) {
            __xlang_vec_push_i32(&table, 0);
            (k = (k + 1));
        }
        int32_t sn = (int32_t)strlen(set1);
        int32_t j = 0;
        while ((j < sn)) {
            (table.data[(int32_t)(unsigned char)(set1[j])] = 1);
            (j = (j + 1));
        }
        int32_t n = (int32_t)strlen(s);
        int32_t i = 0;
        __xlang_sb_new();
        while ((i < n)) {
            int32_t c = (int32_t)(unsigned char)(s[i]);
            if ((table.data[c] == 0)) {
                __xlang_sb_push_char(c);
            }
            (i = (i + 1));
        }
        printf("%s", __xlang_sb_str());
    } else {
        printf("%s", __xlang_str_translate(s, set1, set2));
    }
    return 0;
}
