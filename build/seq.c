...
int32_t main(int argc, char** argv) {
    ...
    int32_t last = 0;
    if (((__xlang_argc_g) == 2)) {
        (last = (int32_t)strtol(__xlang_argv_g[1], 0, 10));
    } else {
        if (((__xlang_argc_g) == 3)) {
            (first = (int32_t)strtol(__xlang_argv_g[1], 0, 10));
            (last = (int32_t)strtol(__xlang_argv_g[2], 0, 10));
        } else {
            (first = (int32_t)strtol(__xlang_argv_g[1], 0, 10));
            (step = (int32_t)strtol(__xlang_argv_g[2], 0, 10));
            (last = (int32_t)strtol(__xlang_argv_g[3], 0, 10));
        }
    }
    if ((step == 0)) {
