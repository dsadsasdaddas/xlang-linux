...
int32_t main(int argc, char** argv) {
    __xlang_argc_g = argc;
    __xlang_argv_g = argv;
    const char * s = __xlang_read_file("/proc/cpuinfo");
    const char * pat = "processor";
    int32_t count = 0;
    int32_t start = 0;
    int32_t n = (int32_t)strlen(s);
    int32_t pn = (int32_t)strlen(pat);
    while ((start < n)) {
        const char * remaining = __xlang_str_slice(s, start, n);
        int32_t pos = __xlang_str_find(remaining, pat);
        if ((pos < 0)) {
            break;
        }
        (count = (count + 1));
