...
int32_t main(int argc, char** argv) {
    ...
    while ((i < n)) {
        if (((int32_t)(unsigned char)(s[i]) == 10)) {
            printf("%s", __xlang_str_concat(__xlang_str_concat(__xlang_int_to_str(lineno), " "), __xlang_str_slice(s, start, i)));
            printf("%s", "\n");
            (lineno = (lineno + 1));
            (start = (i + 1));
        }
    ...
    if ((start < n)) {
        printf("%s", __xlang_str_concat(__xlang_str_concat(__xlang_int_to_str(lineno), " "), __xlang_str_slice(s, start, n)));
        printf("%s", "\n");
    }
    return 0;
}
