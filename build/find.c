...
int32_t find_dir(const char * dir, const char * pat);
int32_t main(int argc, char** argv);

int32_t find_dir(const char * dir, const char * pat) {
    int32_t n = __xlang_dir_count(dir);
    int32_t i = 0;
    int32_t count = 0;
    while ((i < n)) {
        const char * entry = __xlang_dir_entry(dir, i);
        if ((!(strcmp(entry, ".") == 0))) {
            if ((!(strcmp(entry, "..") == 0))) {
                const char * path = __xlang_str_concat(__xlang_str_concat(dir, "/"), entry);
                if (__xlang_is_dir(path)) {
                    (count = (count + find_dir(path, pat)));
                }
                if ((__xlang_str_find(entry, pat) >= 0)) {
                    printf("%s", path);
                    printf("%s", "\n");
                    (count = (count + 1));
                }
...
int32_t main(int argc, char** argv) {
    ...
    const char * pat = "";
    if (((__xlang_argc_g) >= 2)) {
        (dir = __xlang_argv_g[1]);
    }
    if (((__xlang_argc_g) >= 3)) {
        (pat = __xlang_argv_g[2]);
    }
