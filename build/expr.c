...
int32_t main(int argc, char** argv) {
    ...
    int32_t a = (int32_t)strtol(__xlang_argv_g[1], 0, 10);
    const char * op = __xlang_argv_g[2];
    int32_t b = (int32_t)strtol(__xlang_argv_g[3], 0, 10);
    int32_t result = 0;
    if ((strcmp(op, "+") == 0)) {
        (result = (a + b));
    }
    ...
    printf("%s", __xlang_int_to_str(result));
    printf("%s", "\n");
    return result;
}
