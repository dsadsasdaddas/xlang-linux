...
int32_t main(int argc, char** argv) {
    __xlang_argc_g = argc;
    __xlang_argv_g = argv;
    int32_t n = __xlang_dir_count("/proc");
    int32_t i = 0;
    while ((i < n)) {
        const char * entry = __xlang_dir_entry("/proc", i);
        int32_t first = (int32_t)(unsigned char)(entry[0]);
        if ((first >= 48)) {
            if ((first <= 57)) {
                const char * comm = __xlang_read_file(__xlang_str_concat(__xlang_str_concat("/proc/", entry), "/comm"));
                int32_t cn = (int32_t)strlen(comm);
                int32_t cend = cn;
                if ((cn > 0)) {
                    if (((int32_t)(unsigned char)(comm[(cn - 1)]) == 10)) {
                        (cend = (cn - 1));
                    }
                }
                printf("%s", entry);
                printf("%s", " ");
                printf("%s", __xlang_str_slice(comm, 0, cend));
                printf("%s", "\n");
            }
        }
        (i = (i + 1));
