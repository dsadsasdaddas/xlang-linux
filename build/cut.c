...
int32_t main(int argc, char** argv) {
    ...
    __xlang_argv_g = argv;
    if (((__xlang_argc_g) < 3)) {
        printf("%s\n", "usage: cut <delim> <field> [file]");
        return 1;
    }
    const char * delim = __xlang_argv_g[1];
    int32_t field = (int32_t)strtol(__xlang_argv_g[2], 0, 10);
    const char * s = "";
    if (((__xlang_argc_g) >= 4)) {
    ...
    int32_t d = (int32_t)(unsigned char)(delim[0]);
    int32_t n = (int32_t)strlen(s);
    int32_t lstart = 0;
    int32_t i = 0;
    while ((i <= n)) {
        if (((i == n) || ((int32_t)(unsigned char)(s[i]) == 10))) {
            const char * line = __xlang_str_slice(s, lstart, i);
            int32_t ln = (int32_t)strlen(line);
            int32_t fstart = 0;
            int32_t cur = 1;
            int32_t j = 0;
            while ((j <= ln)) {
                if (((j == ln) || ((int32_t)(unsigned char)(line[j]) == d))) {
                    if ((cur == field)) {
                        printf("%s", __xlang_str_slice(line, fstart, j));
                        printf("%s", "\n");
                    }
                    (cur = (cur + 1));
                    (fstart = (j + 1));
