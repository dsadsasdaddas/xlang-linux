...
int32_t main(int argc, char** argv) {
    ...
    __xlang_argv_g = argv;
    if (((__xlang_argc_g) < 3)) {
        printf("%s\n", "usage: split <file> <lines-per-chunk>");
        return 1;
    }
    const char * s = __xlang_read_file(__xlang_argv_g[1]);
    int32_t lpc = (int32_t)strtol(__xlang_argv_g[2], 0, 10);
    int32_t n = (int32_t)strlen(s);
    int32_t fnum = 0;
    int32_t lc = 0;
    ...
    while ((i < n)) {
        if (((int32_t)(unsigned char)(s[i]) == 10)) {
            (lc = (lc + 1));
            if ((lc >= lpc)) {
                __xlang_write_file(__xlang_str_concat("xsplit_", __xlang_int_to_str(fnum)), __xlang_str_slice(s, chunk_start, (i + 1)));
                (fnum = (fnum + 1));
                (lc = 0);
                (chunk_start = (i + 1));
            }
    ...
    if ((chunk_start < n)) {
        __xlang_write_file(__xlang_str_concat("xsplit_", __xlang_int_to_str(fnum)), __xlang_str_slice(s, chunk_start, n));
        (fnum = (fnum + 1));
    }
    printf("%d\n", fnum);
    printf("%s", " files written\n");
    return 0;
}
