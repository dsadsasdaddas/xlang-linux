...
Vec_String split_lines(const char * s);
...
int32_t main(int argc, char** argv) {
    ...
    int32_t limit = 0;
    const char * s = "";
    bool has_limit = (((__xlang_argc_g) >= 3) && (strcmp(__xlang_argv_g[1], "-n") == 0));
    if (has_limit) {
        (limit = (int32_t)strtol(__xlang_argv_g[2], 0, 10));
        if (((__xlang_argc_g) >= 4)) {
            (s = __xlang_read_file(__xlang_argv_g[3]));
        } else {
    ...
    Vec_String lines = split_lines(s);
    int32_t n = ((int32_t)(lines).len);
    if ((limit == 0)) {
        (limit = n);
    }
    int32_t i = (n - 1);
    while ((i > 0)) {
        int32_t j = (int32_t)(rand() % ((i + 1)));
